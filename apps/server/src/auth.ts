...
export class Auth {
  ...
  private async issue(user:User,reply:FastifyReply):Promise<void>{
    const token=randomBytes(32).toString('hex');const expiresAt=Date.now()+this.config.SESSION_DAYS*86400000;
    await this.db.pool.query('INSERT INTO sessions(token_hash,user_id,expires_at) VALUES($1,$2,$3)',[digestToken(token),user.id,expiresAt]);
  ...
  register(app:FastifyInstance):void {
    ...
    app.post('/api/auth/register',async(request,reply)=>{
      ...
      try{await this.db.pool.query('INSERT INTO users(id,username,username_key,password_hash,created_at) VALUES($1,$2,$3,$4,$5)',[user.id,user.username,user.username.toLowerCase(),encoded,Date.now()]);}
      catch(error){if(isPgError(error,'23505'))throw new HttpError(409,'USERNAME_TAKEN','That username is already in use.');throw error;}
      await this.issue(user,reply);return reply.code(201).send({user});
    });
    app.post('/api/auth/login',async(request,reply)=>{
      ...
      const user={id:row.id,username:row.username};await this.issue(user,reply);return {user};
    });
    ...
    app.post('/api/auth/password',async(request,reply)=>{
      ...
      await transaction(this.db,async c=>{await c.query('UPDATE users SET password_hash=$2 WHERE id=$1',[session.user.id,encoded]);await c.query('DELETE FROM sessions WHERE user_id=$1',[session.user.id]);});
      this.onUserSessionsRevoked(session.user.id);
      await this.issue(session.user,reply);return {ok:true};
    });
  }
}
