...
import { createPublisher,publishGame } from './realtime.js';
...
const games=new Games(db,kv,health,lexicon,config);const io=createPublisher(kv);games.publish=state=>publishGame(io,games,state);
...
for(const signal of ['SIGINT','SIGTERM'] as const)process.once(signal,()=>{stopping=true;clearInterval(timer);void(async()=>{while(running)await new Promise(resolve=>setTimeout(resolve,50));await health.stop();await new Promise<void>(resolve=>io.close(()=>resolve()));await kv.close();await db.pool.end();})().then(()=>process.exit(0)).catch(()=>process.exit(1));});
