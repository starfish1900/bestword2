...
export function Game() {
  ...
  useEffect(()=>{
    clearView(); scoreResync.current=true; viewRef.current=null; previousTurn.current=null; latestClockTime.current=0; setLoading(true); setDraft(emptyDraft()); setPending(readPending(gameId)); setReplayIndex(0); setAutoReplay(false); setReplayMode(searchParams.has('replay'));
  ...
  const tileOrigins=replayTileOrigins(game,replayMode?replayIndex:game.moves.length);
