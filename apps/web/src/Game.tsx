...
import { replayBoard, replayScores, replayTileOrigins, type TileOrigin } from './replay';
import { pendingAfterReply } from './pending';
import { useApp } from './store';
import { AnimatedScore } from './AnimatedScore';
import { authHref, requestedReplayPosition } from './authNavigation';
...
export function Game() {
  const {gameId=''}=useParams(); const [searchParams,setSearchParams]=useSearchParams();
  ...
  const [wantsReplay,setReplayMode]=useState(searchParams.has('replay')); const [replayIndex,setReplayIndex]=useState(()=>requestedReplayPosition(searchParams)); const [autoReplay,setAutoReplay]=useState(false);
  const [copyStatus,setCopyStatus]=useState('');
  const [scoreEpoch,setScoreEpoch]=useState(0); const [replaySeekEpoch,setReplaySeekEpoch]=useState(0);
  const scoreResync=useRef(true);
  ...
  const receive=useCallback((next:GameView)=>{
    ...
    if(scoreResync.current){setScoreEpoch(value=>value+1);scoreResync.current=false;}
    ...
    if(previousTurn.current===null&&next.game.historyAccess!=='recent')setReplayIndex(index=>Math.min(index,next.game.moves.length));
    const turn=`${next.game.moveCount??next.game.moves.length}:${next.game.activeSeat}`;
  ...
  useEffect(()=>{
    clearView(); scoreResync.current=true; viewRef.current=null; previousTurn.current=null; latestClockTime.current=0; setLoading(true); setDraft(emptyDraft()); setPending(readPending(gameId)); setReplayIndex(requestedReplayPosition(searchParams)); setAutoReplay(false); setReplayMode(searchParams.has('replay'));
    ...
    const subscribe=()=>{scoreResync.current=true;setConnected(true);setError('');void gameRequest('game:subscribe',gameId).then(reply=>{if(alive)handleReply(reply);}).catch(error=>{if(alive)setError(messageOf(error));});};
  ...
  const canReplay=!!user&&!!game&&game.historyAccess!=='recent';const replayMode=wantsReplay&&canReplay;
  useEffect(()=>{if(!game||!canReplay)return;const params=new URLSearchParams(searchParams);if(replayMode){params.set('replay','1');params.set('move',String(replayIndex));}else{params.delete('replay');params.delete('move');}if(params.toString()!==searchParams.toString())setSearchParams(params,{replace:true});},[canReplay,gameId,replayMode,replayIndex,setSearchParams]);
  ...
  const moveCount=game.moveCount??game.moves.length;const recentMoves=game.recentMoves??game.moves.slice(-3);
  const replaySignIn=authHref('sign-in',`/game/${gameId}?replay=1&move=${wantsReplay?requestedReplayPosition(searchParams):0}`);
  const aiLabel=game.ai?`${game.ai.difficulty[0]!.toUpperCase()+game.ai.difficulty.slice(1)} AI`:null;
  ...
  const tileOrigins=replayMode?replayTileOrigins(game,replayIndex):game.tileOrigins??replayTileOrigins(game,game.moves.length);
  const lastReplayMove=game.moves[replayIndex-1];const lastMove=replayMode?lastReplayMove:recentMoves.at(-1);
  const newlyPlaced=new Set((replayMode?lastReplayMove?.tiles:game.lastMoveTiles??game.moves.at(-1)?.tiles)?.map(tileIndex)??[]);
  ...
  const status=replayMode?`Replay · move ${replayIndex} of ${game.moves.length}`:game.status==='finished'?resultText(game):game.status==='paused'?'Game paused · protecting your progress':initialCountdown?`Ready in ${initialCountdown}…`:game.status==='waiting'?(game.ai?'Waiting for your game connection':'Waiting for both players to connect'):clockExpired?'Confirming result…':game.ai?.seat===game.activeSeat?`${aiLabel} is thinking`:you?(you.seat===game.activeSeat?'Your move':game.players[you.seat].passed?'You have passed · your score is fixed':`${game.players[game.activeSeat].username} is thinking`):`${game.players[game.activeSeat].username} to play`;
  ...
  return <main className="game-shell">
    ...
    <div className="play-layout">
      <section className="arena" aria-label="Game board and players"><div className="players-strip">{([firstSeat,secondSeat] as const).map(seat=><div key={seat} className={`player-card ${game.activeSeat===seat&&game.status==='active'?'active':''} ${you?.seat===seat?'you':''}`}><span className="avatar">{game.players[seat].username[0]?.toUpperCase()}</span><div className="player-info"><strong><span className={`player-swatch player-${seat}`} role="img" aria-label={`${seat===0?'Pale green':'Pale orange'} tiles`} title={`${game.players[seat].username} contributes ${seat===0?'pale green':'pale orange'} tiles`}/>{game.players[seat].username}{you?.seat===seat&&<small> you</small>}</strong><span>{game.ai?.seat===seat&&<b className="ai-level">{aiLabel} · </b>}{replayMode?'Score at this position':game.players[seat].passed?'Passed forever':!game.players[seat].connected?'Disconnected':`${game.players[seat].rackSize} on rack`}</span></div><div className="player-numbers"><AnimatedScore key={`${gameId}:${seat}`} score={displayScores[seat]!} username={game.players[seat].username} mode={replayMode?'replay':'live'} position={replayMode?replayIndex:moveCount} resetKey={`${scoreEpoch}:${replaySeekEpoch}`} connected={connected}/><time className={clockValue(seat)<30000&&!game.players[seat].passed?'urgent':''}>{replayMode?'—':timeText(clockValue(seat))}</time></div></div>)}</div>
      <div className="board-container" ref={boardRef}><BoardGrid board={displayBoard} origins={tileOrigins} playerNames={[game.players[0].username,game.players[1].username]} draft={replayMode?emptyDraft():draft} highlights={newlyPlaced} canAct={canAct} onSelect={index=>{if(!canAct)return;setDraft(previous=>clickSquare(previous,game.board,index));setError('');}}/></div>
      ...
      {wantsReplay&&!canReplay&&game.status!=='finished'&&<p className="replay-access">Sign in to explore earlier moves. <Link to={replaySignIn}>Continue to sign in</Link></p>}
      {replayMode ? <div className="replay-controls"><span className="eyebrow">EVERY MOVE TELLS A STORY</span><h2>{replayIndex===0?'The opening board':lastMove?.word??(lastMove?.action==='PASS'?'Pass forever':'No words')}</h2><p>{replayIndex===0?'Two random words set the scene.':lastMove?`${game.players[lastMove.seat].username} · ${lastReplayMove?.notation??'Turn completed'} · +${lastMove.score} points`:''}</p><input aria-label="Replay move" type="range" min={0} max={game.moves.length} value={replayIndex} onChange={event=>{setAutoReplay(false);setReplaySeekEpoch(value=>value+1);setReplayIndex(Number(event.target.value));}}/><div className="replay-buttons"><button className="button secondary" aria-label="First position" onClick={()=>{setReplaySeekEpoch(value=>value+1);setReplayIndex(0);setAutoReplay(false);}}>⏮</button><button className="button secondary" aria-label="Previous move" disabled={replayIndex===0} onClick={()=>{setReplayIndex(index=>Math.max(0,index-1));setAutoReplay(false);}}>←</button><button className="button primary" onClick={()=>{if(replayIndex===game.moves.length)setReplayIndex(0);setAutoReplay(!autoReplay);}}>{autoReplay?'Pause':'Play'}</button><button className="button secondary" aria-label="Next move" disabled={replayIndex===game.moves.length} onClick={()=>{setReplayIndex(index=>Math.min(game.moves.length,index+1));setAutoReplay(false);}}>→</button><button className="button secondary" aria-label="Last position" onClick={()=>{setReplaySeekEpoch(value=>value+1);setReplayIndex(game.moves.length);setAutoReplay(false);}}>⏭</button></div><span className="replay-count">{replayIndex} / {game.moves.length} moves</span><button className="text-button" onClick={()=>{setReplayMode(false);setAutoReplay(false);}}>{game.status==='finished'?'Back to result':'Back to live game'}</button></div> : game.status==='finished' ? <div className="result-card"><span className="result-flower" aria-hidden="true">✦</span><span className="eyebrow">A GAME WELL PLAYED</span><h2>{resultText(game)}</h2><p>{game.result?.reason==='clock'?'Decided on time.':game.result?.reason==='disconnect'?'Decided after a player disconnected.':game.result?.reason==='both-passed'?'Both players have passed. Every point counted.': 'This game ended without a scored result.'}</p><>{canReplay?<button className="button primary full" onClick={()=>{setReplayMode(true);setReplayIndex(0);}}>Explore the replay <Icon name="history"/></button>:<Link className="button primary full" to={replaySignIn}>Sign in to replay <Icon name="history"/></Link>}</><Link className="button secondary full" to="/">Find another game <Icon name="arrow"/></Link></div> : <>
      ...
      <div className="recent-moves"><div className="control-label"><span>THE STORY SO FAR</span><button className="text-button" onClick={()=>setHistoryOpen(true)}>All moves <Icon name="chevron" size={12}/></button></div>{moveCount===0?<p className="muted">The first move is still yours to imagine.</p>:[...recentMoves].reverse().map(item=><div className="recent-move" key={item.revision}><span>{game.players[item.seat].username}</span><strong>{item.word??(item.action==='PASS'?'PASS':'NO WORDS')}</strong><em>+{item.score}</em></div>)}</div>
      </section></div>
    ...
    {historyOpen&&<Modal title="Every move, every point" onClose={()=>setHistoryOpen(false)} className="history-modal"><div className="seed-words"><span className="eyebrow">OPENING WORDS</span><strong>{game.principalHistory.slice(0,2).join(' × ')}</strong></div>{!canReplay?<p className="replay-access">Sign in to see every move and its scoring details, and explore the replay. <Link className="button primary full" to={replaySignIn}>Sign in to view history <Icon name="history"/></Link></p>:game.moves.length===0?<p className="muted">No moves yet. The board is full of possibility.</p>:<ol className="full-history">{game.moves.map((item,index)=><li key={item.revision}><MoveDetails move={item} game={game} index={index}/>{(game.status==='finished'||!you)&&<button className="text-button" onClick={()=>{setReplaySeekEpoch(value=>value+1);setReplayMode(true);setReplayIndex(index+1);setAutoReplay(false);setHistoryOpen(false);}}>View on board <Icon name="arrow" size={14}/></button>}</li>)}</ol>}</Modal>}
    {help&&<Rules onClose={()=>setHelp(false)}/>}
  </main>;
}
function BoardGrid({board,origins,playerNames,draft,highlights,canAct,onSelect}:{board:Board;origins:TileOrigin[];playerNames:[string,string];draft:Draft;highlights:Set<number>;canAct:boolean;onSelect:(index:number)=>void}) {
  ...
  return <div className={`board-frame ${canAct?'interactive':''}`}><div className="column-labels" aria-hidden="true">{'ABCDEFGHIJKLMNO'.split('').map(letter=><span key={letter}>{letter}</span>)}</div><div className="row-labels" aria-hidden="true">{Array.from({length:15},(_,index)=><span key={index}>{index+1}</span>)}</div><div className="board-grid" role="grid" aria-label="BestWord 15 by 15 board" aria-rowcount={15} aria-colcount={15}>{Array.from({length:15},(_,row)=><div role="row" className="board-row" key={row}>{Array.from({length:15},(_,column)=>{const index=row*15+column;const letter=drafted.get(index)??board[index];const isDraft=drafted.has(index);const origin=origins[index]??null;const originClass=!letter||isDraft?'':origin==='opening'?'opening-tile':origin!==null?`player-${origin}`:'';const originLabel=origin==='opening'?'opening tile':origin!==null?`placed by ${playerNames[origin]}`:'';const cursor=canAct&&draft.cursor===index&&!letter;return <div role="gridcell" key={index} aria-rowindex={row+1} aria-colindex={column+1}><button type="button" id={`square-${index}`} className={`square ${letter?'occupied':''} ${originClass} ${isDraft?'draft-tile':''} ${!isDraft&&highlights.has(index)?'last-played':''} ${cursor?'cursor':''} ${index===112?'center':''}`} aria-label={`${coordinate(index)}, ${letter?`${letter}, ${VALUES[letter]} points${isDraft?', draft':originLabel?`, ${originLabel}`:''}`:'empty'}${cursor?`, ${draft.direction==='H'?'horizontal':'vertical'} cursor`:''}`} aria-disabled={!canAct} onClick={()=>onSelect(index)} onKeyDown={event=>moveFocus(event,index)} tabIndex={index===112||cursor?0:-1}>{letter?<><span>{letter}</span><small>{VALUES[letter]}</small></>:cursor?<span className="cursor-arrow" aria-hidden="true">{draft.direction==='H'?'⇨':'⇩'}</span>:index===112?<span className="center-mark" aria-hidden="true">✦</span>:null}</button></div>;})}</div>)}</div></div>;
}
