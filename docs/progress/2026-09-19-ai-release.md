...
- Local AI probes at 1, 2, 5 and 10 games passed. The final **3,600.063-second mixed run passed every gate** with 10 AI games plus one human game and the whole AI process pinned to one logical CPU. It completed **22,820 AI turns** and 1,055 completed-game cycles, including 2,322 strategic NO WORDS choices. AI latency p95/p99 was 557/750 ms (maximum 1,294 ms); API processing p95/p99 was 8.16/11.47 ms. Peak AI RSS was 312.98 MiB, with no sustained growth across the final five ten-minute windows. No errors, disconnections, incomplete accepted searches, retries, duplicate accepted turns or unexpected forfeits occurred. All saved-state and job/move/receipt checks passed, and both processes exited cleanly. See [AI capacity evidence](ai-capacity.md).

All 29 frozen runtime, harness, lockfile and vocabulary hashes matched after the completed hour. A fresh standalone source checkout installed offline and built successfully, reproducing all 25 compiled runtime/vocabulary artifacts. Completed browser fixtures and load fixtures were cleaned without touching the main local database or Redis DB0.
