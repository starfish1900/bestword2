...
- CI configured for full build/type checking, audit, real PostgreSQL/Valkey integration, Chromium/Firefox/WebKit, Rust graph byte reproduction and complete container smoke startup. It does not deploy or publish an image.
...
Docker/Compose execution, Linux image build, CI hosted run, Render authenticated semantic validation, actual provision/deploy, paid cloud load test, backup drill and production restoration. Docker is unavailable on this Windows machine and was not installed. The local services are real PostgreSQL 18.4 plus Redis 7.2.16, not Valkey 8. These distinctions must remain visible in release claims.
