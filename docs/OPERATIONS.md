...
Use a unique new drill database name if that name already exists. Compare account counts, migration versions, event revisions, command receipts and representative replay results. A successful export alone is not a tested backup. The supplied guarded fixture drill completed a real local round trip and checked all application tables, tile conservation, restored command receipts and the outbox; see [the recorded backup evidence](progress/backup.md). It does not establish managed point-in-time recovery or production recovery time.
