// SHA-256 is used only for reproducible artifact integrity, not authentication.
const K: [u32; 64] = [
...
pub fn digest(input: &[u8]) -> [u8; 32] {
    ...
    for block in padded.chunks_exact(64) {
        ...
        for (dst, value) in h.iter_mut().zip([a,b,c,d,e,f,g,hh]) { *dst = dst.wrapping_add(value); }
    }
    let mut result = [0_u8;32];
    for (i, n) in h.iter().enumerate() { result[i*4..i*4+4].copy_from_slice(&n.to_be_bytes()); }
    result
}

pub fn hex(input: &[u8]) -> String { input.iter().map(|b| format!("{b:02x}")).collect() }

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn official_sha256_vectors() {
