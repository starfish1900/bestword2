...
const examples=specs.map(spec=>{
  ...
  for(const word of result.words){
    assert(lexicon.has(word.word),`${word.word} must be in the actual dictionary`);
    assert([...word.word].some(letter=>isVowel(letter as Letter))&&[...word.word].some(letter=>!isVowel(letter as Letter)),`${word.word} must contain a vowel and a consonant`);
  }
